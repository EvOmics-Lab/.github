# Welcome! to the EvOmics Lab 

 
We are happy to welcome you to the Evolution and Comparative Multi-Omics (EvOmics) Lab, whether you are joining us as an undergraduate student, Master's student, PhD student, postdoctoral researcher, or collaborator. We will try to make your arrival and onboarding as easy as possible. 

As your mentor, my goal is to support your scientific and professional development throughout your time in the lab. I will do my best to provide guidance, encourage your independence, and help you develop the skills to pursue your own research path and career goals. 

This document contains practical information to help you get started in the lab, Center for Quantitative Genetics and Genomics (QGG), Aarhus University, and living in Aarhus. 
 

## 1. General information about the Lab: 

+ **Lab mission:** The EvOmics Lab seeks to combine evolution with gene functional information through the development of computational methods and integrative multi-omics approaches. We investigate how gene duplication, gene loss, pleiotropy, and functional redundancy drive functional innovation and trait evolution across plant species. We are committed to producing open, reproducible computational tools and fostering interdisciplinary collaborations to study plant biology and contribute to future crop improvement. 

+ **Lab vision:** Advance our understanding of how evolutionary processes shape complex traits in plants and to translate this knowledge into innovative solutions for sustainable agriculture. By integrating evolutionary biology, comparative genomics, and artificial intelligence, we aim to uncover the genetic principles underlying crop diversity, adaptation, and resilience, enabling the development of more productive and climate-resilient crops. 

+ **Values:** QGG and our lab welcome people with different backgrounds from all over the world. For that, the values we encourage are respect, collaboration, inclusivity, and communication. Please feel free to come to talk with me or HR if you have any problems at the workplace. 

+ **Lab web page:**  https://evomics.wordpress.com/ 

+ **GitHub:** https://github.com/EvOmics-Lab 

+ **QGG web page:** https://qgg.au.dk/en/1 

 

## 2. Before your first day 

+ **Office address:** C. F. Møllers Allé 3, bld. 1130, 8000 Aarhus 

+ **QGG email:** you will get an account before you join the lab. Follow the instructions to activate and change your user name. 

+ **Important contacts:**

   Irene Julca – Principal investigator (irene.julca@qgg.au.dk) 

   Louise Fischer Koue - Chief Secretary QGG (lfkoue@qgg.au.dk) 

 

## 3. After your arrival 

### 3.1 Building access, key, and card:  

In your fist day you will receive a card to access the building and a key to access the offices. Also, please check your QGG email for your Access Code to the main doors. This code is only needed when the main doors are locked, usually after 5 pm, weekends, or holiday periods. 

### 3.2 Laptop 

You will get a laptop on your first day. It usually has Windows installed, if you need to install Ubuntu, you need to do it yourself. I can help you with that, just let me know. If you decide to do it on your own, here is some information: 

- [x] Prepare a bootable ubuntu USB 

You need to activate Heimdal. This will grant you temporary administrator rights for 30 minute. You can activate Heimdal by right-clicking the blue Heimdal icon in the bottom right corner of your screen and selecting "Privileges App Control - "Request Admin Privileges.” Then, enter your auID@uni.au.dk and AU-password, when you run a program as administrator. For more information, check this link:  https://medarbejdere.au.dk/en/administration/it/guides/security/activate-admin-rights-heimdal 

Get you BitLocker recovery key, you will need each time you boot your laptop in windows. Check this link: https://support.microsoft.com/en-us/windows/security/encryption/find-your-bitlocker-recovery-key 

Turn off BitLocker 

Reboot your laptop and press ESC, F1, F2, F8 or F10 to access the BIOS settings. Change the booting option to start with USB 

Switch off your laptop. Insert the bootable USB and follow the instructions to install Ubuntu. Here you have to make a decision whether you want to have double boot or only Ubuntu. Check this info: https://ubuntu.com/tutorials/install-ubuntu-desktop#1-overview 

Remember, if you selected double boot, when you access Windows, you will be asked to insert the BitLocker key (I still haven’t figured out how to turn it off O_O). 

### 3.3 Cluster 

You will get access to GenomeDK, the first high-performance computing facility for the life sciences in Denmark. Please request an account and check the documentation here: https://genome.au.dk/ 

When requesting the account, state that you are a PhD student or a postdoc in my group and need access to work on some projects. 

I will give you access to shared folders. 


### 3.4 GitHub 

We have a group GitHub, I will give you admin rights once you arrive in the lab. If you are not familiar with it, please check the documentation here: https://docs.github.com/en/get-started/start-your-journey/creating-an-account-on-github 

### 3.5 Slack 

Send me the email that you want to use for Slack.

### 3.4 VPN 

Check this page: https://medarbejdere.au.dk/en/administration/it/guides/network/vpn-remoteaudk 

 

## 4. Lab culture 

Respect and inclusion   

Collaboration and not competition   

Helping each other   

Open science principles   

Code should be shared early 

Negative results are valuable 

Questions are always welcome 

Students are expected to become increasingly independent over time 

 

## 5. Meetings 

Weekly lab meetings (presentations and journal club). 1h 

Weekly one-on-one meetings. I will give you a slot of approx. 30 min to 1h 

Feel free to join the Plant and Genomic program meetings and the QGG Forum meetings. 

  

## 6. Moving to Aarhus 

Please contact the international office: https://international.au.dk/about/contact-and-visitors-service/international-services/ic 

They will provide you with more information and specific to your case. 

To find a house in Aarhus there are different options: 

Student Housing Aarhus: https://international.au.dk/life/locations/housing/auhousing/student-housing-aarhus 

https://www.archouse.dk/ 

https://www.boligportal.dk/ 

https://www.boligsiden.dk/ 

Some Facebook pages: For example, Aarhus international 

 

## 7. Programs 

Conda: https://genome.au.dk/docs/installing-software/ 

FastOMA: https://github.com/DessimozLab/FastOMA 

OMArk: https://github.com/DessimozLab/OMArk 

BUSCO: https://busco.ezlab.org/ 

Hugging Face: https://huggingface.co/ 

Ete4: https://etetoolkit.github.io/ete/ 

 

 

Feel free to add anything else you think necessary 
